...
void OnTick()
{
   // --- Always check for commands (every tick) ---
   CheckCommandFile();
...
{
   Print("Command received: ", jsonCmd);
   
   string action = ExtractJSONString(jsonCmd, "action");
   bool success = false;
   string ticketStr = "";
   
   if(action == "open")
   {
      string type = ExtractJSONString(jsonCmd, "type");
      ...
      int orderType = (type == "sell") ? ORDER_TYPE_SELL : ORDER_TYPE_BUY;
      ulong ticket = PlaceOrder("XAUUSD", orderType, volume, sl, tp, "XAU_Execution");
      
      if(ticket > 0)
      {
         success = true;
         ticketStr = IntegerToString(ticket);
         Print("Order placed: ticket=", ticket, " type=", type, " vol=", volume);
      }
      else
         Print("FAILED to place order: ", type, " vol=", volume);
   }
   ...
   else if(action == "news_pause")
   {
      string enabled = ExtractJSONString(jsonCmd, "enabled");
      newsPause = (enabled == "true" || enabled == "1");
      Print("News pause: ", newsPause);
      success = true;
   }
   else
   ...
   if(resHandle != INVALID_HANDLE)
   {
      ...
      FileWrite(resHandle, "{\"status\":\"" + status + "\",\"action\":\"" + action + "\",\"ticket\":\"" + ticketStr + "\"}");
      FileClose(resHandle);
   }
}
